...
*/
   ...
   // Crashes with EXC_BAD_ACCESS
    ...
    func testChaining() {
        var chain = Bank(array: [1, 2, 3])
        XCTAssertEqual(chain.first().invokeAll() as Int, 1, "Returns first element which ends the chain")

        chain = Bank(array: [1, 2, 3, 4, 5])
        XCTAssertEqualObjects(chain.initial(2).value().invokeAll() as [Int], [1, 2, 3], "Returns flatten array from chaining")

        chain = Bank(array: [1, 2, 3, 4, 5])
        XCTAssertEqual(chain.initial().first().invokeAll() as Int, 1, "Returns flatten array from chaining")

        chain = Bank(array: [1, 2, 3, 4, 5])
        XCTAssertEqualObjects(chain.map({ (elem) in elem as Int * 10 }).value().invokeAll() as [Int], [10, 20, 30, 40, 50], "Returns mapped values")
    ...
    */
    func testLazyChainedMethods(){
        //Create iterator
        let iterator:Bank = Bank(array: [1, 2, 3, 4, 5, 6, 7])
        ...
        while(iterator.countSteps() != 0){
            ...
            case 8:
                ...
                XCTAssertFalse(resultBool,                      "No element less than 1");
                break;
                
            case 7:
                resultBool = iterator.step() as Bool;
                XCTAssertTrue(resultBool,                       "All elements are greater than 0");
                break;
                
            case 6:
                resultArray = iterator.step() as [Int];
                XCTAssertEqualObjects(resultArray,              [1,2,3,4,5], "Last 2 elements removed");
                break;
                
            case 5:
                resultInt = iterator.step() as? Int;
                XCTAssertEqualObjects(resultInt,                1, "First object in array");
                break;
                
            case 4:
                resultInt = iterator.step() as? Int;
                XCTAssertEqualObjects(resultInt,                2, "Second object in array");
                break;
                
            case 3:
                resultInt = iterator.step() as? Int;
                XCTAssertEqualObjects(resultInt,                3, "Third object in array");
                break;
            ...
            case 1:
                resultArray = iterator.step() as [Int];
                XCTAssertEqualObjects(resultArray,              [1,2,3,4,5], "Final array of chained methods");
                break;
